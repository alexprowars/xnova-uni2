...
function calc_capacity(){
	...
	for (i = 200; i < 230; i++){
		...
		if(document.getElementsByName(id)[0]){
			cnt = 1 * document.getElementsByName(id)[0].value;
			cap += cnt * (1 * document.getElementsByName("capacity" + i)[0].value);
			if(cnt > 0){
				tmp = 1 * document.getElementsByName("speed" + i)[0].value;
				if(tmp < sp){
					sp = tmp;
				}
			}
	...
	document.getElementById("allcapacity").innerHTML = cap;
	document.getElementById("allspeed").innerHTML = sp;
}
...
function maxspeed() {
	...
	for (i = 200; i < 230; i++) {
		if (document.getElementsByName("ship" + i)[0]) {
			if ((document.getElementsByName("speed" + i)[0].value * 1) >= 1 && (document.getElementsByName("ship" + i)[0].value * 1) >= 1) {
...
function calculateTransportCapacity() {
	...
	var transportCapacity =  storage() - metal - crystal - deuterium;
