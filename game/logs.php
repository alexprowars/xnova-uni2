...
$Display->addTemplate('jurnal', 'jurnal.php');
...
$MaxLogs = db::query ("SELECT COUNT(*) AS `count` FROM {{table}} WHERE `s_id` = '{$user->data['id']}' AND `mission` = '".$journal."' ".$text_query.";", 'logs', true);

$Logs = db::query("SELECT * FROM {{table}} WHERE `s_id` = '{$user->data['id']}' AND `mission` = '".$journal."' ".$text_query." ORDER BY `time` DESC LIMIT ".$start.",10;", "logs");
...
$Display->assign('parse', $parse, 'jurnal');

display('', "Бортовой журнал");
