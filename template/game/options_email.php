<br>
<form action="?set=options&mode=changeemail" method="post">
<table width="519">
<tr>
	<td class="c" colspan="2">Смена адреса электронной почты</td>
</tr>
<tr>
	<th>Текущий пароль</th>
	<th><input name="db_password" size="20" value="" type="password"></th>
</tr>
<tr>
	<th>Новый EMAIL адрес</th>
	<th><input name="email" size="20" value="" type="text"></th>
</tr><tr>
<th colspan="2">
Смена адреса email осуществляется ручным методом после проверки аккаунта администрацией игры.
</th></tr>
<tr><td colspan="2" class="c"><input type="submit" value="Отправить на модерацию"></td></tr>
</table>
</form>