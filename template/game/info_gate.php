<?=$parse['gate_time_script'] ?>
<form action="?set=jumpgate" method="post">
<table border="1" width="600">
<tbody>
<tr>
	<th>Исходная точка отправления</th>
	<th><?=$parse['gate_start_link'] ?></th>
</tr><tr>
	<th>Пункт назначения</th>
	<th>
		<select name="jmpto">
		<?=$parse['gate_dest_moons'] ?>
		</select>
	</th>
</tr>
</tbody>
</table>
<table width="600">
<tbody>
<tr>
	<td class="c" colspan="2">Выберите отправляемый флот</td>
</tr>
<? if($parse['gate_wait_time']): ?>
<tr>
	<th class="l" colspan="2">
	<table width="100%">
	<tbody>
	<tr>
		<td style="background-color: transparent;" align="center"><?=$parse['gate_wait_time'] ?></td>
	</tr>
	</tbody>
	</table>
	</th>
</tr>
<? endif; ?>
<? foreach($parse['gate_fleet_rows'] AS $fleet): ?>
<tr>
	<th><a href="?set=infos&gid=<?=$fleet['fleet_id'] ?>"><?=$fleet['fleet_name'] ?></a> (<?=$fleet['fleet_max'] ?> всего)</th>
	<th><input tabindex="<?=$fleet['idx'] ?>" name="c<?=$fleet['fleet_id'] ?>" size="7" maxlength="7" value="0" type="text"></th>
</tr>
<? endforeach; ?>
<tr>
	<th colspan="2"><input value="Отправить" type="submit"></th>
</tr>
</tbody>
<?=$parse['gate_script_go'] ?>
</table>